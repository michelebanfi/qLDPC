"""
JAX-optimized version of BP_per_Iteration.py
Best for Apple Silicon (M1/M2/M3) Macs with Metal acceleration.
...
from decoding.beliefPropagationJAX import (
    performBeliefPropagationBatchJAX,
    generate_errors_and_syndromes,
    warmup_jit
)
...
# For Apple Silicon, 500-2000 is usually good
BATCH_SIZE = 1000
...
print("=" * 60)
print("JAX-accelerated BP Simulation")
print("=" * 60)

total_start = time.time()

for code_idx, code_name in enumerate(codes):
    print(f"\n[{code_idx+1}/{len(codes)}] Processing code: {code_name}")
    oc = np.load(f"codes/{code_name}.npz")
    H = oc['Hx'].astype(np.float32)
    Lx = oc['Lx']
    n = len(H[0])
    
    # Warm up JIT for this code size
    print("  Warming up JIT compiler...")
    warmup_jit(H, maxIter=max(iterations))
    ...
    for max_iter in iterations:
        ...
        initialBeliefs = np.array([np.log((1 - errorRate) / errorRate)] * n, dtype=np.float32)
        ...
        for batch_idx in tqdm.tqdm(range(num_batches), desc=f"  iter={max_iter}", leave=False):
            batch_start = batch_idx * BATCH_SIZE
            batch_end = min((batch_idx + 1) * BATCH_SIZE, trials)
            current_batch_size = batch_end - batch_start
            
            # Generate batch
            errors, syndromes = generate_errors_and_syndromes(
                H, errorRate, current_batch_size, rng
            )
            
            # Run batched BP with JAX
            detections, converged, llrs_batch = performBeliefPropagationBatchJAX(
            ...
            for i in range(current_batch_size):
                ...
                detection = detections[i].astype(np.int8)
                llrs = llrs_batch[i]
...
np.savez('data/BP_per_Iteration_JAX.npz', results=results, allow_pickle=True)
...
plt.tight_layout()
plt.savefig('media/BP_per_Iteration_JAX.png')
print("Plot saved to media/BP_per_Iteration_JAX.png")
